...
ggplot(prices, aes(x = date)) +
  ...
  geom_ysidehistogram(aes(x = after_stat(density)* 15.4944, y = ratio * 15.4944),
...
ggsave('Figures/Figure3_bu.tif', width = 7, height = 3, units = 'in', dpi = 300)
ggsave('Figures/Figure3_bu.png', width = 7, height = 3, units = 'in', dpi = 300)
...
data_46_58 <- TW_discount %>% filter(TW >= 46 & TW <= 58)
data_51_58 <- TW_discount %>% filter(TW >= 51 & TW <= 58)
data_53_58 <- TW_discount %>% filter(TW >= 53 & TW <= 58)
...
eq_46_58 <- paste0('Slope = ', round(coef_46_58[2], 2), ' $/lb')
eq_51_58 <- paste0('Slope = ', round(coef_51_58[2], 2), ' $/lb')
eq_53_58 <- paste0('Slope = ', round(coef_53_58[2], 2), ' $/lb')
...
#  geom_line(aes(y = Average_Discount), size = 0.8, linetype = 'dashed', color = 'black') +  
  ...
  scale_y_continuous(name = 'Price discount ($/bu)', breaks = seq(0, -3.4, by = -0.5)) +
  ...
  annotate('segment', x = 58, xend = 58, y = min(TW_discount$Discount, na.rm = T), yend = 0, 
           linetype = 'dashed', color = 'black', size = 0.2, alpha = 0.5) +
  
  annotate('segment', x = 46, xend = 46, y = -3.2, yend = -1.84, 
           linetype = 'dashed', color = 'black', size = 0.2, alpha = 0.5) +
  annotate('segment', x = 51, xend = 51, y = -2.7, yend = -1.02, 
           linetype = 'dashed', color = 'black', size = 0.2, alpha = 0.5) +
  annotate('segment', x = 53, xend = 53, y = -2.2, yend = -0.69, 
           linetype = 'dashed', color = 'black', size = 0.2, alpha = 0.5) +
  
  # Regression lines
  stat_smooth(data = data_46_58, method = 'lm', se = F, size = 0.4, linetype = 'dotted',
              geom = 'line', alpha = 0.5, color = 'black') +
  stat_smooth(data = data_51_58, method = 'lm', se = F, size = 0.4, linetype = 'longdash',
              geom = 'line', alpha = 0.5, color = 'black') +
  stat_smooth(data = data_53_58, method = 'lm', se = F, size = 0.4, linetype = 'solid',
              geom = 'line', alpha = 0.5, color = 'black') +
  
  # Regression equations
  annotate('text', x = 56, y = -3.1, label = eq_46_58, size = 3, family = 'Times New Roman') +
  annotate('text', x = 56, y = -2.6, label = eq_51_58, size = 3, family = 'Times New Roman') +
  annotate('text', x = 56, y = -2.1, label = eq_53_58, size = 3, family = 'Times New Roman') +
  
  # Double arrows for intervals
  annotate('segment', x = 46, xend = 58, y = -3.2, yend = -3.2, #linetype = 'dotted',
           arrow = arrow(ends = 'both', length = unit(0.1, 'inches')), size = 0.4) +
  annotate('segment', x = 51, xend = 58, y = -2.7, yend = -2.7, #linetype = 'dashed',
           arrow = arrow(ends = 'both', length = unit(0.1, 'inches')), size = 0.4) +
  annotate('segment', x = 53, xend = 58, y = -2.2, yend = -2.2, #linetype = 'solid',
           arrow = arrow(ends = 'both', length = unit(0.1, 'inches')), size = 0.4) +
...
ggsave('Figures/Figure4_bu.tif', width = 7, height = 3, units = 'in', dpi = 300)
ggsave('Figures/Figure4_bu.png', width = 7, height = 3, units = 'in', dpi = 300)
