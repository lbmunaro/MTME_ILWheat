...
# Print model info
print(paste('convergence =', MTME.z_rr6aprc.asr$converge))
MTME.z_rr6aprc.asr$trace |>
  as.data.frame() |> rownames_to_column('Iteration') |>
  filter(Iteration=='LogLik') |> print()

# Save
save.image('Data/MTME.z_rr6aprc.RData')

# Update model ----
MTME.z_rr6aprc.asr <- update_asreml(MTME.z_rr6aprc.asr, 
                                 max_updates = 50,
                                 save_path = 'Data/MTME.z_rr6aprc.RData')
