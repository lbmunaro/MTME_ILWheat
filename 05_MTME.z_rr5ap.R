...
# Print model info
print(paste('convergence =', MTME.z_rr5ap.asr$converge))
MTME.z_rr5ap.asr$trace |>
  as.data.frame() |> rownames_to_column('Iteration') |>
  filter(Iteration=='LogLik') |> print()

# Save
save.image('Data/MTME.z_rr5ap.RData')

# Update model ----
MTME.z_rr5ap.asr <- update_asreml(MTME.z_rr5ap.asr, 
                                 max_updates = 20,
                                 save_path = "Data/MTME.z_rr5ap.RData")