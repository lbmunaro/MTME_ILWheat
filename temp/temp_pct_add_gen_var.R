...
vc <- summary(MTME_RR3.asr)$varcomp

vc |> as.data.frame() |> rownames_to_column() |> View()
...
Gvar <- Lambda_a_star |> as.data.frame() |> cbind(R) |>
  ...
  rowwise() %>%
  mutate(
    var_V1 = 100 * (V1^2) / (V1^2 + V2^2 + V3^2 + R),
    var_V2 = 100 * (V2^2) / (V1^2 + V2^2 + V3^2 + R),
    var_V3 = 100 * (V3^2) / (V1^2 + V2^2 + V3^2 + R),
    var_T = var_V1+var_V2+var_V3
  ) %>%
  ungroup() |>
  #  mutate(var_T = sum(var_V1,var_V2)) |>
  arrange(desc(var_T))
