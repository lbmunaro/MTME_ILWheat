...
load('HPC_MTME_SI_ILWheat/STSE_mod.RData')

print(STSE.asr$call)

STSE.asr <- update(STSE.asr, workspace = '40gb')
...
save.image('HPC_MTME_SI_ILWheat/STSE_mod.RData')


# summary(STSE.asr)$varcomp |> as.data.frame() |>
#   rownames_to_column('TraitEnv') |>
#   filter(str_detect(TraitEnv, "TraitEnv:vm\\(Gkeep, Ginv.sparse\\)!")) |>
#   mutate(TraitEnv = str_remove(TraitEnv, "TraitEnv:vm\\(Gkeep, Ginv.sparse\\)!TraitEnv_")) |>
#   select(TraitEnv, component) |>
#   rename(Vg = component)
