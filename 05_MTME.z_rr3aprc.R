...
# Print model info
print(paste('convergence =', MTME.z_rr3aprc.asr$converge))
MTME.z_rr3aprc.asr$trace |>
  as.data.frame() |> rownames_to_column('Iteration') |>
  filter(Iteration=='LogLik') |> print()

# Save
save.image('Data/MTME.z_rr3aprc.RData')

# Update model ----
MTME.z_rr3aprc.asr <- update_asreml(MTME.z_rr3aprc.asr, 
                                 max_updates = 50,
                                 save_path = 'Data/MTME.z_rr3aprc.RData')
